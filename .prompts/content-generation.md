...
**⚠️ 중복 방지 프로세스 (반드시 순서대로 수행)**:

1. **블랙리스트 확인 (1단계)**
   - 이 프롬프트의 **맨 아래**로 스크롤하여 "🚫 절대 사용 금지 주제" 섹션을 찾으세요
   - 해당 섹션에 나열된 **모든 주제를 완전히 숙지**하세요
   - 목록을 한 번만 보지 말고, **2-3번 반복해서 확인**하세요

2. **유사성 체크 (2단계)**
   - 금지 목록과 다음 기준으로 비교하세요:
     - **동일한 동사**: "주문하기", "계획하기", "찾기" 등이 겹치면 제외
     - **동일한 장소**: "카페", "레스토랑", "도서관" 등이 겹치면 제외
     - **동일한 상황**: "친구와 ~", "가족과 ~" 등이 겹치면 제외
     - **동일한 행위**: "환불", "문의", "상담" 등이 겹치면 제외
   - **예시**: 블랙리스트에 "카페에서 음료 주문하기"가 있으면
     - ❌ "베이커리에서 빵 주문하기" (주문 행위 중복)
     - ❌ "카페에서 와이파이 비밀번호 물어보기" (카페 중복)
     - ✅ "영화관 예매하기" (완전히 다른 주제)

3. **창의적 주제 생성 (3단계)**
   - 블랙리스트를 **완전히 회피**하는 새로운 주제를 생성하세요
   - 아래 "주제 선택 원칙"을 따르세요
...
**🎲 주제 다양화 전략**:
- **장소**: 병원, 우체국, 은행, 공항, 호텔, 박물관, 미용실, 세탁소, 동물병원, 주유소, 수리점 등
- **활동**: 예약, 상담, 수리, 교환, 가입, 취소, 연장, 분실신고, 민원제기, 협상 등
- **관계**: 이웃, 동료, 상사, 점원, 서비스 직원, 전문가, 낯선 사람 등
- **상황**: 문제 해결, 정보 요청, 의견 제시, 협력, 거절, 설득, 위로 등
...
#### 1단계: 한글 스크립트
- **분량**: 5-8문장 **강력 권장** (학습 부담 최소화)
  - **가중치**: 5-6문장 (최우선), 7-8문장 (권장), 9-10문장 (희박하게)
  - **절대 원칙**: 12문장 이상은 절대 금지
- **번호 매기기**: 각 문장 앞에 "1. ", "2. " 형식 사용 금지
- **스타일**: 자연스러운 대화/상황 스크립트
- **학습 포인트 마크업**: `**한글표현**{외국어표현}` 형식
  - 문장당 0-2개 선별 (없으면 생략 가능)
  - **반드시 회화에서 재사용 가능한 의미 있는 표현만 선별**
  - **🚨 중요**: {외국어표현}은 **강조된 부분만** 번역 (전체 문장 번역 금지)
  - **올바른 예시**:
    - ✅ `우리 **서프라이즈 파티를 열어주는 거 어때**{throw him a surprise party}?`
    - ✅ `누구를 초대할지 **명단을 만들어야 해**{make a guest list}.`
  - **잘못된 예시**:
    - ❌ `우리 **서프라이즈 파티를 열어주는 거 어때**{How about we throw him a surprise party}?` (전체 문장 번역)
    - ❌ `**비밀로 해야 한다는 거야**{we have to keep it a secret from Minjun}` (앞뒤 문맥 포함)
...
- **초급**: 기본 어휘, 단순 문장구조, 일상 상황 (5-8문장 권장)
- **중급**: 비즈니스 어휘, 복합 문장구조, 업무 상황 (5-8문장 권장)
- **고급**: 전문 어휘, 고급 문법, 추상적 주제 (5-8문장 권장)

## 품질 보장 체크리스트

- ✅ **UUID 사용**: 입력 파라미터의 UUID를 topic_id에 정확히 사용
- ✅ **문장 수 적정성**: 5-8문장 권장 (12문장 이상 절대 금지)
